...
| `/app/today` | `src/pages/TodayPage.jsx` via `AppShell.jsx` | rituel du jour : `next_action` live + dernière entrée journal + impact/proof ; future surface `Zen` | `next_action`, `insights`, `step`, `journal_entries`, `sessions`, `quota` | Supabase direct client | réel |
...
Transition produit actée :
- `Today` et `Aujourd'hui` désignent aujourd'hui la même surface runtime
- cette surface doit être transformée en `Zen`
- la transformation doit réutiliser l'état déjà branché, pas repartir d'une page vide
  - fil en cours
