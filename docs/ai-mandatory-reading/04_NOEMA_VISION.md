...
- Un Today consommant `next_action` live + dernière entrée journal, qui sert désormais de base à la future surface `Zen`
...
La vision produit garde 4 surfaces coeur :
...
Dans le runtime actuel, `Zen` n'existe pas encore comme surface finale nommée.
La bonne lecture produit est donc :

- `Today` = surface actuelle branchée
- `Zen` = évolution officielle de `Today`

Autrement dit, Noema ne doit pas ajouter une surface en plus.
Noema doit transformer progressivement `Today` en `Zen` en conservant ce qui marche déjà :
...
| Today consommant les données réelles, base de transition vers `Zen` | réel |
...
### Priorité 5 — Transformer `Today` en `Zen`

La vision manuscrite de `NOEMA_VISION2.md` demande une vraie `Page Zen`.
Le bon mouvement n'est pas d'ajouter un nouvel onglet.
Le bon mouvement est de transformer la surface `Today` existante en `Zen`.

Ce que ça demande :
- conserver les sources runtime déjà branchées (`next_action`, dernière session, journal, preuve, impact)
- rendre la surface plus calme, plus immersive, plus rituelle
- ajouter un exercice ou un rituel adapté au travail en cours
- faire de `Zen` un prolongement réel du couple `Chat -> Journal`

### Priorité 6 — Semantic memory pour la détection de patterns longs
...
Mais c'est à construire après les priorités 1-5.
