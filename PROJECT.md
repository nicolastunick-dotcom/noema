...
| 02/04/2026 | Claude Code | Sprint 4 — session live : `sessionIdRef = crypto.randomUUID()` au mount dans AppShell ; `session_id` propagé dans chaque appel `callAPI()` ; `saveSession()` transformé INSERT → UPSERT sur `sessions.id` (une seule ligne par session active) ; `api_usage.session_id` écrit côté Sonnet (Greffier l'écrivait déjà) ; 0 migration SQL. | ✅ | Reprise cross-refresh hors périmètre pour l'instant |
