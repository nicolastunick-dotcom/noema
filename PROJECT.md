...
| 02/04/2026 | Claude Code | Sprint 4.1 (anticipé) — session live minimale : `sessionIdRef = crypto.randomUUID()` au mount dans AppShell ; `session_id` propagé dans chaque appel `callAPI()` ; `saveSession()` transformé INSERT → UPSERT sur `sessions.id` (une seule ligne par session active) ; `api_usage.session_id` écrit côté Sonnet (Greffier l'écrivait déjà) ; 0 migration SQL. Reclassé en 4.1 pour préserver Sprint 4 = réalignement UX réel. | ✅ | Sprint 4 UX réel reste à faire |
| 02/04/2026 | Codex | Mise à jour de `DEBATE.md` et `RETENTION.md` — nouveau débat entre les agents déjà présents, cette fois ancré sur `docs/system/` et l’évolution réelle post-alignement (Sprint 1 exécuté, noyau plus cohérent, surfaces `Journal` / `Today` encore en retard). | ✅ | Utiliser ces addendums comme lecture qualitative de la trajectoire produit, pas comme source de vérité runtime à la place des documents système |
| 02/04/2026 | Codex | Clôture documentaire Sprint 4 — réalignement UX avec l’état réel du runtime : `Success` relit `subscriptions.status`, `App.jsx` lui passe `user` + `sb`, `Landing` / `Pricing` ne survendent plus `Journal` / `Today`, `TodayPage` et `JournalPage` assument explicitement leur état statique / aperçu. | ✅ | Plus aucune promesse produit critique ne dépasse la réalité backend visible. Sprint 5 est débloqué côté documentation. |
| 02/04/2026 | Codex | Extension expérimentale de `DEBATE.md` — simulation qualitative de 5 jeunes de 18 à 28 ans débattant des mêmes sujets produit, avec mention explicite du caractère purement expérimental et non représentatif. | ✅ | Utiliser ce bloc comme matériau de réflexion UX / positionnement, pas comme validation utilisateur réelle |
| 02/04/2026 | Claude Code | Sprint 6 — continuité UX : `TodayPage` : CTA "Passer à l'action" sur l'intention, bouton "Commencer une conversation" si pas de données, badge "Jour X de ton parcours" ; `JournalPage` : label "Intention du jour", badge progression, placeholder guidé (3 questions), section "Ce que tu retiens", feedback inline "Entrée enregistrée" post-save ; `AppShell` : prop `onChat` ajoutée à `TodayPage`. 0 appel LLM, 0 nouvelle table. | ✅ | — |
